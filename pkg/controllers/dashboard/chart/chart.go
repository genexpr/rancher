...
func (r *RancherConfigGetter) GetPriorityClassName() (string, error) {
	return r.getKey(PriorityClassKey, settings.ConfigMapName.Get())
}

// GetWebhookValues attempts to retrieve the desired webhook Values from the rancher-config configMap.
func (r *RancherConfigGetter) GetWebhookValues() (map[string]any, error) {
	strVal, err := r.getKey(WebhookChartName, CustomValueMapName)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal '%s' value: %w", WebhookChartName, err)
	}
