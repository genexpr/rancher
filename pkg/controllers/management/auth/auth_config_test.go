	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			controller := authConfigController{
				cleanup:     &service,
				authConfigs: newMockAuthConfigClient(config),
			}
			...
			obj, err = controller.authConfigs.Get(config.Name, metav1.GetOptions{})
			require.NoError(t, err)
			u, _ := obj.(runtime.Unstructured)
			authConfig := u.UnstructuredContent()
			assert.Equal(t, test.newAnnotationValue, authConfig["metadata"].(map[string]any)["annotations"].(map[string]any)[CleanupAnnotation])
		})
	}
}
